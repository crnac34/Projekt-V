...
<body class="gradient-background">
  <div class="flex items-center justify-center h-screen">
    <div class="flex flex-col items-center p-4">
      ...
      <h2 id="valentineQuestion" class="text-4xl font-bold italic text-[#bd1e59] my-4">Zelis li biti maj valentajn?</h2>
  ...
  <script type="module">
    ...
    noButton.addEventListener('click', function() {
      if (noClickCount < 5) {
        ...
        buttonHeight += 70; // Increase height by 5px on each click
        buttonWidth += 70;
        fontSize += 50; // Increase font size by 1px on each click
        yesButton.style.height = `${buttonHeight}px`; // Update button height
        yesButton.style.width = `${buttonWidth}px`;
        yesButton.style.fontSize = `${fontSize}px`; // Update font size
        if (noClickCount < 6) {
          noButton.textContent = ["Ne", "Si sigurna?", "Vita molim tee", "NE MOZE!!", "PREMISLI SI", "Plakal se bum", "Jos jedna sansa!"][noClickCount];
        }
      }
    });
    ...
    yesButton.addEventListener('click', () => {
      ...
      valentineQuestion.textContent = "Jessss!!! VOLIM TEEE"; // Change the question text
